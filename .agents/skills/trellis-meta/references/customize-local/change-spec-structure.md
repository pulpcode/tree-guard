...
| Need | Edit location |
| --- | --- |
| Add or split a TreeGuard backend/test convention | `.trellis/spec/backend/` |
...
```bash
python3 ./.trellis/scripts/task.py add-context <task> implement ".trellis/spec/backend/security-boundaries.md" "Security conventions"
python3 ./.trellis/scripts/task.py add-context <task> check ".trellis/spec/backend/security-boundaries.md" "Security review rules"
```

## Change Monorepo Packages

TreeGuard currently has no package map. Add `packages` and `default_package` to
`.trellis/config.yaml` only after real independent packages exist.
