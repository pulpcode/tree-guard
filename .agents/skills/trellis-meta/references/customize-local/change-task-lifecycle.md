...
If you fork `task.py` to add a new creation path (e.g. an external import that
bypasses `cmd_create`), audit whether your path also calls `set_active_task`.
Without that call, your created tasks will not surface as active. The installed
status behavior is documented in `.trellis/workflow.md` under "Current-task
mechanism" and "Phase Index."
