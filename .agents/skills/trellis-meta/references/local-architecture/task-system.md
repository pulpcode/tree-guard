...
TreeGuard requires `--no-commit` so archive changes remain unstaged for review.
When modifying the task system, the AI should prefer script commands to maintain
structure. Edit JSON/Markdown directly only when scripts do not cover the need.
