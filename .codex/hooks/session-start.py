...
def _get_task_status(trellis_dir: Path, hook_input: dict) -> str:
    ...
    if task_status == "completed":
        return (
            f"Status: COMPLETED\nTask: {task_title}\nSource: {active.source}\n"
            "Next: Load `trellis-finish-work`; it must archive with "
            f"`python3 ./.trellis/scripts/task.py archive {task_dir.name} --no-commit` "
            "and record only a sanitized journal entry with `--no-commit`."
        )
