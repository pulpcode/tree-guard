...
- [x] 独立准备并冻结 H2 新树、36条候选和28条执行集；
- [x] 冻结 R2 A：Recall@20=16/20、Recall@8=16/20、MRR=0.775、
  非字面 Recall@20=6/10，且 hard negative 与显式空目标均为4/4；
- [x] 实现隔离本地 Provider、冻结 profile、独立 H2 索引/查询/候选合同、`0600`
  不可覆盖索引，以及 aggregate-only B runner；H1 合同与默认依赖保持不变；
- [x] 无语义 smoke 冻结 batch=16，并验证相同输入向量重放一致、CPU 内存和加载耗时
  可接受；零模型 B 预检重放 A 一致，并在任何 B 输出前补冻词面基线
  Recall@8=4/4；
