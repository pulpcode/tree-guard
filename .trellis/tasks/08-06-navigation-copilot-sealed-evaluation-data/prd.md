# 构建导航 Copilot 密封 clean-room 数据 v3

## Goal

在功能基线 `cdf3250629ffcbddb9bd9f0c461a89a23affcd4b` 上，由新的独立 clean-room
构造主体从空白建立一棵未参与当前产品调参的虚构 resource 信息树，以及 Navigation
...
- v3 必须使用新的 worktree、任务执行主体、namespace、seed、稳定 ID 规则和生成蓝图；
- v3 构造主体不得读取 v1/v2 工作树、fixture、生成器、场景、Oracle、审核、preflight、
  Prompt、模型请求/响应或逐项审阅结论；只允许读取本 PRD、功能基线公开合同与项目规范；
- 本 PRD 中记录的 v2 聚合失败类别只用于门禁设计，不授权按旧数据逐项改写；
- 当前审阅主体只负责冻结合同和后续 data-commit 审核，不参与 v3 数据生成。
...
- 树包含 6—10 个语义连贯顶层分支、4—6 层深度、同层近邻、跨分支近义、节点种类冲突、
  值类型/基数冲突和多个可接受目标；
- 禁止使用“维度列表 × 指标列表”、对象列表 × 同一子树模板、名称列表 × 账册模板等
  乘积扩充节点；不得仅替换名称、ID 或顺序制造“不同”蓝图；
- 生成器必须为每个非叶蓝图记录本任务内的稳定 blueprint ref。规范子树签名排除名称、
  稳定 ID 和 sibling 顺序，保留节点 kind、值类型、基数、子节点角色及递归结构；含至少
  5 个后代的蓝图实例中，单一签名最多出现 3 次，重复签名覆盖的实例比例不得超过 20%；
  preflight 同时报告签名总数、唯一数、最大重复组和重复实例比例；
- 每个二至四层非叶子树必须能用一条独立的父子语义说明解释；Codex Silver 按蓝图抽样
  复核至少 24 个不同签名，禁止用节点计数或结构 hash 代替语义审核；
...
- [ ] 不存在列表乘积造树、无语义 filler、未知父节点、重复 sibling label 或不一致节点合同；
  含至少 5 个后代的规范子树签名最大重复组不超过 3，重复实例比例不超过 20%，且至少
  24 个不同签名已完成语义抽样审核；
...
- [ ] canary 证明 Oracle、目标 node ID、期望状态和评分答案不进入公开 scenario、模型输入
  构造或 manifest 公共视图；
- [ ] preflight 拒绝来源、节点规模、VALUE、蓝图重复率、配额、类别、重复子集、澄清
  对照、弱证据原因、审核、Oracle、hash、权限、额外字段、bool-as-int 和冻结字节篡改；
