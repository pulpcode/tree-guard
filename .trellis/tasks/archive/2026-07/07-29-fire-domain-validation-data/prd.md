...
- 其他项目只可作为待核验的参考来源；来源、许可和敏感性未确认的内容不得导入
  本仓库。
...
- 采用来源方案 A：高层覆盖类别只参考权威公开来源，节点名称、层级、需求和
  模型输出全部独立虚构。
...
- [x] 故障 oracle 清单声明非法 JSON、多余字段、缺失字段、超时、429/500、
  重试、截断和虚构泄漏 canary 的预期；实际 transport 行为由既有测试验证。
...
- 把其他项目的现有树、期望值、训练 JSONL、模型缓存或 Git 历史直接复制到
  本仓库。
- 生产 Patch、Gold 标注、embedding、向量数据库或生产效果声明。
- 访问其他项目的 `.env`、凭据历史、模型原始响应和诊断缓存。
...
- 其他项目中的材料必须先做来源级筛选，不能把“当前文件可读”等同于“允许
  派生导入”。

## Research References

- [`research/fire-domain-sources.md`](research/fire-domain-sources.md) —
  主题语义只从权威公开来源取材，并独立虚构所有验证数据。

## Feasible Approaches

### A. 公开概念重新取材并独立构造（推荐）

- 只借鉴公开、可复述的方法论，不导入其他项目工件。
...
**Context**：可参考的其他项目材料没有明确许可证，部分评测资产也可能已被开发
过程消费；TreeGuard 规范禁止把真实消防字段或其稳定伪名写入 fixture。
...
- 合并后全量 Python `unittest`：199 tests passed。
- 合并后前端 Vitest：2 files、7 tests passed。
