...
### D-012：在线默认最多两次顺序 LLM 调用

1. 自然语言到意图卡；
2. 小候选集合到语义建议。

Patch、Diff、校验和影响分析使用确定性代码。
后续 D-044 为意图需要一次澄清的路径增加一个受约束例外：回答后重新编译意图，
因此该路径最多三段顺序调用。
...
### D-044：意图澄清采用每轮一问、MVP 最多一轮

初始 `ChangeIntentDraft` 返回 `NEEDS_CLARIFICATION` 时，禁止直接生成
`CONFIRM_FOR_RETRIEVAL`。用户回答以初始 `draft_hash` 绑定实际查看的唯一问题，
模型根据原需求、初始意图、问题和回答重新生成完整意图。修订意图无问题时才能人工
确认并进入检索；仍有问题时固定为 `CLARIFICATION_LIMIT_REACHED`，转人工调查。

当前合同只实现第一轮，不把“每轮只问一个问题”固化成整个产品永远只能追问一次。
