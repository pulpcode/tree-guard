...
Clean-room 数据集。它只模拟消防任务治理的功能形状，不对应真实消防信息树、真实
单位、真实设施、真实事故、真实字段或工程规范。

数据只使用“任务基本信息、现场态势、人员与组织、救援力量与装备、处置过程、
特殊任务扩展”等常识级业务概念，并从零构造字段、层级、需求、模型输出和
oracle；没有复制其他项目的树、expected、Gold、训练 JSONL、模型缓存或 Git
历史，也不声明这些概念构成行业标准或完整消防 Schema。

当前内容版本为 `FFV-*-V2`。V2 替换了 V1 中“鸣镜、辉纹、雾径、泉幕”等只有
虚构性、没有明确业务含义的造词，并同步改写了模型实际读取的需求文本。扩展节点
按所属子树的业务对象生成，字段切面同时决定值类型和基数，不再把随机名称和随机
类型组合在一起。
...
场景需求正文描述实际治理问题，例如共享的人员属性是否应在特殊任务子树重复
新增、特殊任务人员资质应归入人员主干还是场景扩展。`scenario_ref` 只是技术引用，
不会再作为自然语言需求提交给模型。`NO_CANDIDATES` 使用一个明确标记为域外的
ASCII 技术占位字段，以避免中文字符级基线产生偶然词法命中。
