# TreeGuard 安全边界与评测方案

状态：设计基线
适用阶段：外网开发、单向导入、内网文件型 Shadow MVP

## 1. 安全目标

TreeGuard 必须同时满足：

1. 真实业务数据、内部代码和模型交互记录不离开内网；
2. 外网开发产物可以离线、可核验地单向导入；
3. LLM 无数据库凭证、无任意查询工具、无正式写权限；
4. 所有模型输出经过结构化校验和确定性风险门；
5. 内网测试失败时，可以通过白名单诊断信息协作，而不泄露敏感内容；
6. Shadow MVP 在物理上不存在生产数据库写路径。

## 2. 双区资产边界

| 外网通用 Core 可以持有 | 必须留在内网 |
|---|---|
| `TreeSnapshot`、`ChangeIntent`、`Overlay`、`Patch` 等抽象合同 | 真实树、节点名称、路径和 ID |
| 通用工作流、校验器、Diff、Trace 框架 | 真实历史版本和版本说明 |
| 完全虚构的领域数据和 Mock Adapter | Spring Boot 源码、内部 DTO、API 地址 |
| 契约测试、回放、评测和 Conformance 工具 | 邮件模板、Usage 真实数据和 VALUE |
| 不含真实业务内容的 Prompt 模板 | 含业务内容的 Prompt、模型输出和 Trace |
| 标准错误码和指标定义 | 真实 Gold、专家讨论和审批记录 |
| 锁定的依赖、SBOM、许可证清单 | 数据库凭证、密钥、网络拓扑和安全策略 |

外网通用 Core 不应包含任何仅适用于真实消防树的硬编码规则。

## 3. 单向导入包

外网交付包至少包含：

- 源码；
- 版本化 JSON Schema；
- File Adapter 和 Mock Adapter；
- 完全虚构的 fixtures；
- 单元测试、契约测试和端到端测试；
- Conformance CLI；
- 依赖锁文件；
- 可选 wheelhouse 或离线容器包；
- SBOM；
- 许可证清单；
- `SECURITY.md`；
- Transfer Manifest；
- 每个文件的校验值。

安全审查应覆盖：

- 源码和依赖扫描；
- 禁止遥测和自动更新；
- 运行时无外联；
- 无隐藏数据收集；
- 无数据库写连接器；
- 锁定所有传递依赖；
- wheel 或容器摘要可重复校验。

SBOM 只是可核验材料，不等于安全结论。

## 4. 内网向外网的诊断白名单

默认只允许：

- 固定枚举错误码；
- `PASS / FAIL`；
- 契约版本、通用包版本；
- 分桶后的数量、耗时、延迟和指标；
- 一次性随机案例编号；
- 经人工重写到无关虚构领域的最小复现。

禁止直接外传：

- 异常自由文本和堆栈；
- 内部目录、包名和类名；
- 真实节点名称、路径和 ID；
- Prompt、模型输出和 Trace；
- 数据库查询结果；
- 对低熵业务字段生成的确定性哈希；
- 全树一致的假名映射。

完整 `TreeSnapshot`、`TreeDiff` 和 `HistoryReview` 都属于内网敏感制品。TreeDiff 的字段变化需要服务内网专家审查，因此 `before/after` 可能包含真实名称、label、remark、extension 或尚未分类的 metadata；HistoryReview 还包含节点引用、VALUE 外层观察信号和来源哈希。它们不得直接作为跨网诊断输出，也不得仅靠删除 VALUE 后就视为可外传。

跨网协作只允许输出固定错误码和聚合结果，例如变更类型计数、ERROR/WARNING 数量、HistoryReview 风险/门禁计数和分桶指标；不得输出 node delta、字段前后值、节点引用、snapshot/diff/run hash。若未来 CLI 增加完整 Diff 或 HistoryReview 输出，必须明确标为内网命令，并与聚合报告分离。

当前 `snapshot/diff/case/run hash` 都是无密钥的确定性摘要，只用于一致性、关联和回放校验，不是数字签名或来源认证。可信流程必须通过 `verify_history_run_against_snapshots` 从受控快照重新计算 HistoryReview 并逐字段比对；若未来接收跨进程或跨系统制品，还需要访问控制及独立的签名/MAC 机制，不能因为“哈希能对上”就信任内容。

低熵节点名称可能被字典反推。即使哈希看似不可读，也不能默认视为安全脱敏。一次性随机案例编号不得外传内部映射表。

## 5. 数据访问等级

模型默认只访问 Schema，不访问运行时 `VALUE`。

建议定义：

```text
SCHEMA_ONLY
AGGREGATED_EVIDENCE
REDACTED_SAMPLE
RAW_VALUE
```

- `SCHEMA_ONLY`：默认等级，名称、路径、类型、基数和审批后的 Overlay；
- `AGGREGATED_EVIDENCE`：只提供分桶统计或规则结果；
- `REDACTED_SAMPLE`：经过专项审批的最小脱敏样本；
- `RAW_VALUE`：MVP 禁止，未来需要单独授权、审计和保留策略。

Trace 尽量保存引用和版本信息，而不是复制敏感值。

## 6. 威胁模型与确定性门禁

### 6.1 Prompt 注入

树节点名称、描述、版本说明和文档文本都属于不可信输入。它们可能包含诱导模型改变目标或越权调用工具的文字。

缓解：

- 数据和系统指令在消息结构中隔离；
- LLM 只获得白名单候选和只读上下文；
- 无数据库凭证；
- 无任意工具调用；
- Provider 使用 JSON Object 模式，输出仍必须通过本地精确合同校验；
- 所有动作由确定性代码重新验证。

### 6.2 编造节点 ID

缓解：

- 每次候选召回生成受控引用；
- 模型只能引用候选集合；
- Patch 编译器重新校验 `node_id`、版本和哈希；
- 未知 ID 立即失败，不尝试自动修复。

### 6.3 版本与索引过期

缓解：

- 固定基础树、Overlay、检索索引和 Usage Manifest 版本；
- 任一版本变化时将建议和 Patch 标记为 `STALE`；
- 过期依赖索引不得显示“无影响”；
- Patch 应带节点级前置条件和幂等键。

### 6.4 检索漏召回导致错误新增

缓解：

- 全树查重；
- 多通道宽召回；
- 单独评测召回和语义决策；
- 检索健康度不足时禁止从“未找到”推断“应该新增”；
- 高风险新增必须人工确认。

### 6.5 模型超时和格式异常

缓解：

- 无澄清路径最多两次、单轮澄清路径最多三次顺序调用；
- 小候选上下文；
- 严格 Schema 输出；
- 有限重试；
- 超时、非法输出或不一致均转 `ABSTAIN` 或人工处理。

### 6.6 外部百炼开发冒烟

外部百炼不在真实数据的内网信任边界内。它只用于完全虚构或经组织审批允许出域的
脱敏案例，不能发送真实树、真实节点文本或路径、内部版本说明、VALUE、邮件 Usage、
Prompt/Trace 或内部接口信息。

EvidencePack 的外部模型投影会排除真实 `node_id`、内部 run/case/pack 哈希、业务版本
标识、原始 VALUE 和未分类扩展字段，但仍保留语义审查所需的名称和路径，因此不能把
“通过投影”自动等同于“允许出域”。冒烟 CLI 必须同时收到 `--live` 与
`--external-data-approved` 才能调用云服务。

API Key 从进程环境读取；本地开发也可使用当前工作目录中被 Git 忽略的 `.env`，
但该文件必须是 `0600` 普通文件、不得为符号链接，且进程环境优先。生产环境仍应
使用密钥管理服务。密钥不得写入配置样例、日志、Trace、测试制品或 Git。一旦在
聊天、终端记录或其他非密钥系统中暴露，必须撤销并换新。模型响应只按
`JSON_OBJECT` 能力处理，本地精确校验失败最多重试一次，随后 `ABSTAIN`；不得把
云端结果包装成内网 FP8 部署的等价性证明。

Provider 只接受官方已列出的 DashScope 共享主机或固定地域的
`[workspace-id].[region].maas.aliyuncs.com` 形态，并要求标准
`/compatible-mode/v1` 路径。HTTP 重定向被禁用，避免 `Authorization` 被转发到其他
目标；进程环境中的 HTTP(S) proxy 也不会被继承。响应和敏感文件使用严格 JSON
解析，重复键、`NaN`/无穷数、超长整数、超限载荷和非普通文件全部拒绝。完整内部
输出以 `0600`、拒绝覆盖和拒绝最终符号链接的方式新建。

专家原文是新的独立敏感载荷，不能继承 EvidencePack 初审时的一次性出域确认。
`treeguard-expert-review prepare-approval` 先在不联网的情况下生成一个私有
`PENDING` 清单，其哈希覆盖准确专家原文、EvidencePack、初审草稿、官方端点、
模型、Prompt、生成参数以及首次/重试请求体。审批人必须保留请求文件并另存
`APPROVED` 清单；实际调用同时要求 `--external-data-approved` 与
`--external-approval-file`。Provider 在联网前重算精确哈希，会话回放也重算并校验，
且 `approved_at` 必须不晚于 AI 事件时间。请求不包含 actor、时间、会话状态、最终
理由、内部 ID 或内部哈希。真实专家讨论默认只进入内网 Qwen；外部百炼只处理完全
虚构或组织明确批准的脱敏内容。授权失败必须发生在网络调用前。

该清单只能证明“一个可回放文件声明了审批且哈希覆盖本次请求”，不能认证
`approved_by`。当前 action actor、审批人和初始 AI 来源分别标记为
`UNVERIFIED_FILE_ASSERTION` / `UNVERIFIED_FILE_BUNDLE`；正式环境仍需要服务端身份、
ACL 和不可篡改审计。

专家原文、AI 整理和专家裁决分别保存，不允许 AI 回写原文或隐式晋升状态。完整
会话仍使用 `0600`、拒绝覆盖和拒绝最终符号链接的内部文件；树导出、bundle、
action、上一会话和审批清单输入也必须不宽于 `0600`。聚合输出不包含文本、证据
引用、actor、版本、路径、ID 或哈希。

### 6.7 Shadow 自动化偏见

缓解：

- AI 建议先生成并冻结；
- 人工在看不到 AI 建议时独立作答；
- 人工作答冻结后再揭示并比较；
- 专家分歧进入仲裁或 ambiguity 集。

当前 `ExpertReviewSession` 固定标记为 `ASSISTED`。因为专家在裁决前可以看到 AI
整理，该结果不得标为 `REAL_HUMAN_GOLD`。建立冻结 Gold 时仍应使用盲评流程：先
冻结独立人工作答，再揭示 AI 草稿并比较。辅助审查数据可以衡量接受、修订和拒绝
比例，但不能冒充独立模型评测标签。

v1 每个会话最多记录一次 AI 整理。AI 整理后新增的专家思考可以继续进入人工状态
与最终裁决，但不会在原会话再次调用模型；若确需新的 AI 整理，应创建新会话并明确
来源关系。

### 6.8 事件账本完整性边界

会话使用来源绑定的 genesis hash、事件前序哈希和完整 session hash，回放可发现
原文或理由被改写、事件删除、复制、重排、状态伪造以及 pack/draft 来源漂移。
这些都是无密钥 SHA-256；拥有完整写权限的攻击者可以重算整条链，因此不能宣传为
密码学不可抵赖。正式跨进程信任仍需要最小权限 ACL，并在风险评估后增加 MAC、
签名、审计存储或 WORM。

文件模式还没有权威 HEAD、跨进程 compare-and-swap 或全局 action registry。同一
输入会话可以产生多个分别完整、分别可回放的后继分支，聚合报告因此明确返回
`authoritative_head_status=NOT_AVAILABLE_FILE_MODE`。只有未来受控仓库/数据库选定
head 并记录 supersession 后，某个分支才能被视为权威；文件回放成功本身不具备该
语义。任何 `APPROVED` 分支仍然 `patch_eligible=false`、`gold_eligible=false`。

### 6.9 Trace 泄露

缓解：

- Trace 只保存在内网；
- 独立权限；
- 静态和传输加密；
- 明确保留期限；
- 导出报告只包含聚合指标。

## 7. 样本来源与用途

所有案例必须标注来源：

```text
REAL_HUMAN_GOLD
EXPERT_APPROVED_SYNTHETIC
AI_SYNTHETIC
DETERMINISTIC_STRESS
```

### 7.1 真实历史数据

历史版本可用于：

- 重建变更血缘；
- 发现疑似重复和结构问题；
- 提出待审查案例；
- 构建专家重新裁决的候选池。

历史版本不能直接作为 Gold，因为端点 Diff 只说明已观察快照之间的净变化；存在修订缺口时连中间操作都不能还原，更不能证明变化正确。

### 7.2 AI 合成数据

可以用于：

- JSON Schema 契约测试；
- 边界和异常测试；
- 压力和性能测试；
- 从已审批案例派生扰动；
- 开发阶段的检索和流程调试；
- 少量 few-shot 示例。

不能用于：

- 替代真实专家 Gold；
- 证明消防业务准确率；
- 由同一个模型同时生成、裁判并宣布正确；
- 改写或扩充冻结测试集后继续调参。

同一来源案例的所有变体必须留在同一个数据分区，避免训练/开发/测试泄漏。

## 8. 真实 Gold 的建立

一条合格 Gold 应满足：

1. 使用变更发生前的树快照；
2. 需求和必要上下文足够完整；
3. AI 建议已生成并冻结，但在人工作答前隐藏；
4. 领域专家独立判断业务主体、角色、场景和边界；
5. 信息树建设人员检查动作、路径、类型和基数；
6. 高影响或分歧案例由第二人复核或仲裁；
7. 最终结论显式标记为 `APPROVED`；
8. 记录可接受候选、正确动作、必要追问和拒答条件；
9. 冻结后不得用于调 Prompt、阈值或检索。

专家仍拿不准的案例不能强行标成确定动作，应进入：

- `DELIBERATING`；
- `NEED_EVIDENCE`；
- ambiguity/challenge 集。

一次发布可能修改不同子树。确定性代码先基于变化节点之间的直接父子关系形成结构候选簇，并避免用根节点或移动节点桥接不同语义域；专家随后合并或拆分候选，确认哪些变化才构成独立案例。修订存在缺口时只能审查端点净变化，不能声称还原了中间操作。

## 9. 评测指标

### 9.1 检索

- `Candidate Recall@5`；
- `Candidate Recall@20`；
- 正确候选的平均排名；
- 按词法重复、路径冲突、场景扩展等案例类型分组报告。

`Recall@5` 只统计 Gold 中确实存在可复用目标的案例。

### 9.2 选择性决策

- `Action Precision`：AI 明确作答案例中的正确比例；
- `Coverage`：AI 明确作答的案例比例；
- `Abstention Quality`：证据不足时是否正确追问或拒答；
- 高风险错误数：
  - 将不同语义错误复用；
  - 将已有语义错误新增；
  - 编造节点 ID；
  - 生成非法 Patch。

置信门不能直接使用 LLM 自报分数。应基于开发集校准检索召回、排序差距、结构一致性和规则信号。

### 9.3 人机协作

- 专家决策时间中位数变化；
- 专家修订意图卡的比例；
- AI 建议被接受、修订和拒绝的比例；
- 从 `OPEN` 到 `APPROVED` 的讨论轮次；
- 用户对推荐理由完整性的评价。

时间指标必须使用配对任务测量，不能只依赖主观估计。

### 9.4 工程与安全

- Qwen P50/P95 延迟；
- 超时率；
- 结构化输出失败率；
- Trace 关键步骤完整率；
- 过期 Patch 拦截率；
- 重复提交幂等性；
- 未授权外联次数；
- 正式数据库写入次数。

## 10. MVP 目标与解释

试点目标：

- `Candidate Recall@5 ≥ 90%`；
- `Action Precision ≥ 85%`；
- `Coverage ≥ 50%`；
- 专家决策时间中位数下降 `≥ 40%`；
- 非法、越权或过期 Patch 放行数 `= 0`；
- 未授权外联数 `= 0`；
- 正式数据库写入数 `= 0`；
- Trace 关键步骤完整率 `= 100%`。

30 条冻结案例只能支撑 MVP 可行性判断。在 50% Coverage 下，动作准确率可能只基于约 15 个明确回答案例，因此报告必须给出：

- 分子和分母；
- 样本类型分布；
- 置信区间；
- 被拒答案例；
- 全部高风险错误。

这些指标是进入“待审批 Patch 试点”的门槛，不是自动发布资格。

## 11. Go / No-Go

硬门槛：

- 至少 30 条真实冻结案例；
- 未授权外网访问为 0；
- 正式数据库写路径为 0；
- 非法或越权 Patch 通过校验为 0；
- Trace 关键步骤完整率 100%；
- 版本过期、模型异常、脏数据和 Prompt 注入均能安全拒绝；
- Transfer Manifest、依赖哈希和 SBOM 可核验。

质量指标未达到时：

- 继续 Shadow；
- 不扩大治理范围；
- 不增加写权限；
- 根据错误归因分别修复召回、排序、Prompt、Overlay 或规则；
- 不以微调模型作为默认补救。
